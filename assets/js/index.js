...
class FractalClock {
    ...
    document.addEventListener('keydown', e => {
      switch (e.keyCode) {
        ...
        case 40:
          ...
          break;
        case 67:
          this.dom.canvas.style.backgroundColor = `rgb(${Math.random() * 255}, ${Math.random() * 255}, ${Math.random() * 255}`;
  ...
  draw() {
    ...
    this.ctx.lineTo(centre.x + Math.cos(this.angle.hour) * length / 2, centre.y + Math.sin(this.angle.hour) * length / 2);
    this.ctx.stroke();
  ...
  drawMinuteSecond(count, length, centre, alpha, angle) {
    ...
    this.ctx.beginPath();
    this.ctx.moveTo(centre.x, centre.y);
    this.ctx.lineTo(centre.x + Math.cos(this.angle.second + angle) * length, centre.y + Math.sin(this.angle.second + angle) * length);
    this.ctx.stroke();

    this.ctx.beginPath();
    this.ctx.moveTo(centre.x, centre.y);
    this.ctx.lineTo(centre.x + Math.cos(this.angle.minute + angle) * length, centre.y + Math.sin(this.angle.minute + angle) * length);
    this.ctx.stroke();

    if (count) {
      this.drawMinuteSecond(count - 1, length * this.settings.scale, { x: centre.x + Math.cos(this.angle.second + angle) * length, y: centre.y + Math.sin(this.angle.second + angle) * length }, alpha * this.settings.opacity, this.angle.second - this.angle.hour - Math.PI + angle + OFFSET);
      this.drawMinuteSecond(count - 1, length * this.settings.scale, { x: centre.x + Math.cos(this.angle.minute + angle) * length, y: centre.y + Math.sin(this.angle.minute + angle) * length }, alpha * this.settings.opacity, this.angle.minute - this.angle.hour - Math.PI + angle + OFFSET);
    }
  }

  buildCanvas() {
    this.dom.body.appendChild(this.dom.canvas);
    this.dom.canvas.style.backgroundColor = `rgb(${this.settings.colour.background[0]}, ${this.settings.colour.background[1]}, ${this.settings.colour.background[2]})`;
